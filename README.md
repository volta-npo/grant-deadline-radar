...
# 💸 Grant Deadline Radar

### A simple open calendar system for tracking grant deadlines, owner tasks, and student deliverables.
...
[Live app](https://volta-npo.github.io/12-grant-deadline-radar/) · [Report an issue](https://github.com/volta-npo/12-grant-deadline-radar/issues) · [Volta](https://voltanpo.org)
...
**Grant Deadline Radar** is a polished, local-first open-source tool from Volta's open-source program. It helps Finance pods managing multiple grant applications turn real community work into structured evidence, client-safe handoffs, and mentor-reviewable release packets.
...
Missed deadlines are common when teams rely on spreadsheets and scattered reminders.

### The niche

Student-led grant writing operations.

### North-star metric

`grant submissions completed before deadline`
...
flowchart TD
    ...
    Evidence --> Workbench["Grant Deadline Radar Calendar"]
    Workbench --> Score["Funding readiness"]
    Score --> Cert["release certification"]
    Cert --> A1["ICS calendar"]
    Cert --> A2["Task CSV"]
    Cert --> A3["Submission readiness report"]
...
flowchart LR
    subgraph Modules["First-class modules"]
        M1["Deadline calendar"]
        M2["Document request checklist"]
        M3["Reviewer queue"]
        M4["Submission readiness states"]
    end
    M1 --> Workbench
    M2 --> Workbench
    M3 --> Workbench
...
sequenceDiagram
    ...
    participant T as Grant Deadline Radar
...
|---|---|
| **Domain workbench** | A purpose-built calendar interface for a simple open calendar system for tracking grant deadlines, owner tasks, and student deliverables. |
...
|---:|---|---|
| 1 | **Deadline calendar** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 2 | **Document request checklist** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 3 | **Reviewer queue** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 4 | **Submission readiness states** | Converts field work into repeatable, reviewable Volta delivery evidence. |
...
This release adds a SaaS-grade operating layer so **Grant Deadline Radar** can stand alone as more than a checklist. It models:
...
```bash
git clone https://github.com/volta-npo/12-grant-deadline-radar.git
cd 12-grant-deadline-radar
npm install
npm test
